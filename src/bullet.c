...
void bullet_new(Bullet *bullet, BulletMetaData metaData)
{
  ...
  bullet->metaData.owner->isHide = metaData.isHide;
}

bool bullet_isEmpty(Bullet *bullet)
{
  return (uint_8)bullet->position.x == (uint_8)EMPTY_BULLET_TEMPLATE.position.x && (uint_8)bullet->position.y == (uint_8)EMPTY_BULLET_TEMPLATE.position.y && bullet->metaData.speed == EMPTY_BULLET_TEMPLATE.metaData.speed;
}
...
void bullet_update(Bullet *bullet)
{
  ...
  if (bullet->metaData.senderType == BulletSenderType_Enemy)
  { // отправитель - противник
    Vec2 playerPosition = player_getInstance()->sprite->position;
  ...
  if (bullet->metaData.senderType == BulletSenderType_Player)
  { // отправитель - игрок
