...
{
    if (bv1->shape == bv2->shape == SPHERE)
    {
        Vec2 dir = vec2_fromPoints(*bv1->position, *bv2->position);
        float distance = vec2_getLength(dir);

        if (distance < bv1->size.x || distance < bv2->size.x)
        {
            return true;
        }
    }
    else if (bv1->shape == bv2->shape == BOX)
    {
